...
maxWidth=0,maxHeight=0;window.onload=function(){var holder=document.getElementById('holder'),patternDict={},hoverDown=function(){down(this,true);},draggerDown=function(){down(this);},hoverUp=function(){up(this);},draggerUp=function(){if(!dragging&&this.type=="text"){location.href=this.data('url')+".html";}
isClicked=false;dragging=false;},down=function(a,hover=false){if(isClicked)return;if(!hover)isClicked=true;if(a.type!="text"){a=a.data('text');a.attr('cursor','move');a.animate({'fill':textColor},200,">");a.node.style.textDecoration='none';}
...
text.data('shape').drag(move,draggerDown,draggerUp);text.drag(move,draggerDown,draggerUp);text.data('shape').hover(hoverDown,hoverUp);text.hover(hoverDown,hoverUp);text.toFront();}
